...
def test_monte_carlo_size_matches_request(fixed_settings: Settings) -> None:
    ...
    result = run_monte_carlo(
        MonteCarloInput(distribution=distribution, sample_size=200, settings=fixed_settings)
    )
...
def test_monte_carlo_is_reproducible(fixed_settings: Settings) -> None:
    ...
    first = run_monte_carlo(
        MonteCarloInput(distribution=distribution, sample_size=50, settings=fixed_settings)
    )
