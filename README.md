# Probabilidad y Estadística

[![Jupyter Book Badge](https://img.shields.io/badge/jupyter-book-orange?style=for-the-badge&logo=data:image/png%3Bbase64,)](https://elc.github.io/probabilidad/)
[![badge](https://img.shields.io/badge/launch-binder-579aca.svg?style=for-the-badge&logo=data:image/png;base64,)](https://mybinder.org/v2/gh/ELC/probabilidad/main)
[![badge](https://img.shields.io/badge/Open-In%20Colab-579aca?style=for-the-badge&logo=googlecolab)](https://colab.research.google.com/github/ELC/probabilidad/blob/main/book/chapters/00_how_to_use.ipynb)

Libro digital de **Probabilidad y Estadística** construido con [Jupyter Book 2](https://jupyterbook.org/) y [MyST](https://mystmd.org/). Incluye notebooks ejecutables, despliegue en GitHub Pages y entorno reproducible en Binder.

Sitio publicado: [elc.github.io/probabilidad](https://elc.github.io/probabilidad/)

## Desarrollo local

Requiere Python 3.13 y [uv](https://docs.astral.sh/uv/).

```bash
uv sync --all-groups
uv run poe build-book      # compila HTML con notebooks ejecutados
uv run poe serve-book       # compila y abre vista previa en http://localhost:8000
uv run poe ci               # pre-commit + compilación (igual que CI)
```

## Binder local (opcional)

```bash
uv run poe build-docker
docker run --rm -p 8888:8888 probabilidad:binder
```
